...
| `langsmith-evaluation` | langsmith, langsmith trace, langsmith dataset, langsmith experiment | harbor | `langsmith-skills:langsmith-trace` | Load after evidence, and only when the tracing or the dataset already lives in LangSmith. Use evals:evals-start for tool-agnostic evaluation methodology and langchain-skills:eval-engineering for Harbor benchmark work; this entry owns nothing beyond the LangSmith workspace. Let the selected skill choose between langsmith-trace, langsmith-dataset, and langsmith-evaluator. | Report integration pending: langsmith-skills, which no installer installs by default. Continue with evals:evals-start for methodology and the web capability for the LangSmith product contract; never claim that the unavailable skill was used, and never assume a LangSmith account or its data exist. |
