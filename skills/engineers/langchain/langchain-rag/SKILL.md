...
description: |
  Pipeline de RAG (retrieval-augmented generation) em LangChain. Cobre document loaders,
  RecursiveCharacterTextSplitter, embeddings (OpenAI) e vector stores (Chroma, FAISS, Pinecone).
  Corpo em inglês, vendorizado de langchain-ai/langchain-skills (fiel ao upstream).
  Use quando: (1) Construir um sistema de RAG com LangChain, (2) Configurar chunking/embeddings,
  (3) Integrar um vector store ao pipeline de retrieval.
  Triggers: /langchain, RAG langchain, retrieval, vector store, embeddings langchain, document loader.
type: knowledge
---
