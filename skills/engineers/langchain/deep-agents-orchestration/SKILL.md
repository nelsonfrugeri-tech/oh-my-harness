...
description: |
  Orquestração multi-agent em Deep Agents. Cobre SubAgentMiddleware (subagents), TodoList para task
  planning e interrupts de human-in-the-loop (HITL). Corpo em inglês, vendorizado de
  langchain-ai/langchain-skills (fiel ao upstream).
  Use quando: (1) Coordenar subagents num Deep Agent, (2) Planejar tarefas com TodoList,
  (3) Inserir aprovação humana (HITL) no fluxo de orquestração.
  Triggers: /deep-agents, deep agents orchestration, subagent, SubAgentMiddleware, task planning.
type: knowledge
---
