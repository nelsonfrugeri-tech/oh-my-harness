...
description: |
  Human-in-the-loop em LangGraph. Cobre interrupt(), Command(resume=...), workflows de
  approval/validation e a estratégia de error handling em 4 camadas. Corpo em inglês, vendorizado
  de langchain-ai/langchain-skills (fiel ao upstream).
  Use quando: (1) Pausar um grafo para aprovação humana, (2) Implementar review/validação manual,
  (3) Retomar execução com Command(resume=...), (4) Estruturar error handling em LangGraph.
  Triggers: /langgraph, human in the loop, interrupt langgraph, approval workflow, Command resume.
type: knowledge
---
