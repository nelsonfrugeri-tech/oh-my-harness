...
description: |
  Persistência e memória em LangGraph. Cobre checkpointers, thread_id, time travel, cross-thread
  memory via Store e os modos de checkpointer scoping em subgraphs. Corpo em inglês, vendorizado
  de langchain-ai/langchain-skills (fiel ao upstream).
  Use quando: (1) Persistir state de um grafo, (2) Manter memória de conversa entre threads,
  (3) Fazer time travel no histórico, (4) Configurar checkpointer em subgraph.
  Triggers: /langgraph, checkpointer, thread_id, langgraph memory, persistence, time travel.
type: knowledge
---
