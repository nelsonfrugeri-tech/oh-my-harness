...
- **Por entidade/endereço**: estenda o mesmo parser do primeiro frontmatter para comparar,
  após normalização, `entities`, `aliases`, `entity_refs`, `references`,
  `occurred_at` e `temporal_refs`; nunca substitua esse parsing por grep no corpo.
  Para repositories, examine também `work/projects/*/context.md` e aplique o gate de
  segurança da entrada direta antes de devolver qualquer endereço.
- **Por assunto**: grep por termos no `title`/`description`/`summary` do
...
3. **Filtros no servidor** — domain/topic/type/knowledge_type/data/archived, entidades,
   endereços, temporalidade e provenance via
