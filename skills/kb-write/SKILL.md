...
```yaml
generated:
  by: knowledge-base/4.2        # o agent que escreveu
...
`domain`, `topic`, `distillation_key`, `created_at`, `summary`, `path`
(absoluto), `supersedes`, `archived: false`, `entities`, `aliases`,
`entity_kinds`, `entity_keys`, `reference_targets`, `occurred_at`,
`temporal_values`, `harness`, `session_id`, `session_name`, `app_name`, `cwd`,
`transcript_path`, `machine_id`, `machine_label`, `hostname` e `username`.
Derive os campos flat dos metadados estruturados no disco. Os campos nullable permanecem
