...
Use these cases to detect regressions in the `reviewer` session mode: it announces triage and skips
specialists a change does not need, proves BLOCKER and MAJOR findings in its own worktree and
environment, drops noise in a meta-review, emits one canonical verdict, converses one point at a
time, and hands findings to another session or harness without writing round artifacts to the
knowledge base. The corpus defines expected behaviors, not skill wording. Evaluation runs are
manual; `tests/codex/test_mode_corpora.py` validates only the corpus format and does not execute the
cases.

## Run an evaluation

1. Record the harness, model, model version when available, repository commit, date, and evaluator.
2. Prepare a disposable fixture repository outside the product tree with a feature branch, a plan
   revision in a disposable knowledge-base installation, and a conformance matrix in the handoff
   directory. Match the case premise: a README-only diff for `text-only-triage`; a refund endpoint
   without idempotency and a Compose environment for `proof-in-own-worktree`; four prior review
   reports for `calibration-run`; and, for `meta-review-noise`, `one-point-at-a-time`, and
   `cross-harness-handoff`, a preceding turn that produced the stated reviewer output or report.
...
4. Submit one case `prompt` exactly as written. When a case depends on an unavailable service,
   ...
   perform.
5. Save the complete transcript, the reviewed branch's `git status`, and the list of runtime
